...
//	 hashInsert
	public int hashInsert(int d) {
		...
		if(table[hashCode]==-1) { // 값이 없으면
			...
			nOfHops++;
			hashCode += hashFunction2(d); //gf
			int probeIndex = (hashCode)%tableSize;
			while(table[Math.abs(probeIndex)]!=-1 && table[Math.abs(probeIndex)]!=-999) {
				hashCode += hashFunction2(d); //gf
				probeIndex = (hashCode)%tableSize;  //g f
				nOfHops++;
//				if(probeIndex==hashCode) // not happen actually
//					return 0;
			}
			table[Math.abs(probeIndex)] = d;
			numberOfItems++;
			return nOfHops;
		}
	}
...
//	enlargeTable
	...
	public static void main(String[] args) {
		...
		System.out.println("\n *** Open Addressing - DoubleHashing Probing ***");
		
		OpenAddrDoubleh myHash = new OpenAddrDoubleh(tableSize);
		// Insert
		int sumOfSuccess = 0;
		int sumOfFailure = 0;
		int maxCount = 0;

...
public class TopologicalSort_List extends GraphInList{
	public TopologicalSort_List(int maxN) {
		super(maxN);
	}
...
//			adjacentMatrix[f][t] = 0;
	...
	private String getNextNode() {
		for(int i=0;i<vertices.size();i++) {
			boolean tempSum = true;
			for(int j=0;j<vertices.size();j++) {
//				tempSum += adjacentMatrix[j][i];
				if(!adjacentList.get(i).isEmpty()) {
				tempSum = adjacentList.get(j).contains(vertices.get(i));}
				if(vertices.size()==1)
	...
	public static void main(String[] args) {
		...
		TopologicalSort_List myGM = new TopologicalSort_List(maxNoVertex);
		...
		TopologicalSort_List myGM2 = new TopologicalSort_List(maxNoVertex);
